...
import { Star, Search, Clock, ArrowRight, MapPin } from "lucide-react";
...
const Poojas = () => {
  ...
  const filtered = (poojas ?? []).filter((p) => {
    ...
    const matchLocation = selectedLocation === "All Locations" || 
      (p as any).location === selectedLocation || 
      !(p as any).location; // show all if no location set
                  ...
                  <div className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-all duration-500 border border-border hover:border-primary/30">
                    <div className="relative overflow-hidden h-56">
                      ...
                      <div className="absolute top-3 right-3 px-3 py-1 rounded-full bg-gradient-gold text-primary-foreground text-xs font-body font-semibold">
                        {Math.round((1 - pooja.price / pooja.originalPrice) * 100)}% OFF
                      </div>
                      <div className="absolute inset-0 bg-gradient-to-t from-temple-brown/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                    </div>
                    <div className="p-5">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-body text-xs text-muted-foreground">({pooja.reviews})</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div>
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className="font-display text-xl font-bold text-foreground">₹{pooja.price.toLocaleString()}</span>
                            <span className="font-body text-sm text-muted-foreground line-through">₹{pooja.originalPrice.toLocaleString()}</span>
                          </div>
                        </div>
                        <motion.div
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          className="p-3 rounded-xl bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300"
                        >
